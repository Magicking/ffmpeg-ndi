...
static int ndi_write_video_packet(AVFormatContext *avctx, AVStream *st, AVPacket *pkt)
{
    ...
    NDIlib_send_send_video_async_v2(ctx->ndi_send, ctx->video);
...
static int ndi_setup_video(AVFormatContext *avctx, AVStream *st)
{
    ...
    if (st->sample_aspect_ratio.num) {
        ...
        av_reduce(&display_aspect_ratio.num, &display_aspect_ratio.den,
                  ...
                  1024 * 1024);
        ctx->video->picture_aspect_ratio = (double)display_aspect_ratio.num / display_aspect_ratio.den;
    }
    else
...
#define NDI_MUXER_LONG_NAME "Network Device Interface (NDI) output using NewTek library"
#else
#define NDI_MUXER_LONG_NAME NULL
#endif

const FFOutputFormat ff_libndi_newtek_muxer = {
    .p.name           = "libndi_newtek",
    .p.long_name      = NDI_MUXER_LONG_NAME,
